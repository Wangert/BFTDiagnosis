...
pub trait ProtocolBehaviour {
    ...
    ///
    fn init_timeout_notify(&mut self, _timeout_notify: Arc<Notify>) {}
    ...
    fn extra_initial_start(
        &mut self,
        _consensus_nodes: HashSet<PeerId>,
        _current_peer_id: Vec<u8>,
    ) -> PhaseState {
    ...
    ///
    fn view_timeout_handler(&mut self, _current_peer_id: PeerId) -> PhaseState {
        let mut queue = VecDeque::new();
        queue.push_back(SendType::Broadcast(vec![]));
        PhaseState::ContinueExecute(queue)
    }
    ...
    ///
    fn is_leader(&self, _current_peer_id: Vec<u8>) -> bool {
        false
    }
    ...
    ///
    fn generate_serialized_request_message(&self, _request: &Request) -> Vec<u8> {
        vec![]
    }
    ...
    ///
    fn check_taken_request(&self, _request: Vec<u8>) -> bool {
        false
    }
    ...
    /// It sets the request.
    ///
    fn set_current_request(&mut self, _request: &Request) {}
}
