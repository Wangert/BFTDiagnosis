...
use crate::{
    behaviour::{PhaseState, ProtocolBehaviour, SendType},
...
use utils::coder::{self, serialize_into_json_str};

pub struct ProtocolActuator<TProtocol>
where
    ...
    TProtocol: Default + ProtocolBehaviour,
{
    ...
    pub async fn no_running_controller_message_handler(&mut self, message: Message) {
        match message.interactive_message {
            ...
            InteractiveMessage::ConfigureParams(param) => {
                println!("收到Param设置消息！");
                match param {
    ...
    pub fn malicious_trigger(&mut self, phase: u8, send_type: SendType) {
        ...
        let current_id = self.peer_id().to_bytes();
        match self.mode() {
            //在此处补充恶意行为
            ...
            ConsensusNodeMode::Dishonest(MaliciousMode::LeaderDelaySendMessage(internal,delay, p)) => {
                ...
                // else if p == phase && !self.protocol_mut().is_leader(current_id.clone()) {
                //     sleep(Duration::from_millis(delay));
                //     self.send_message(send_type);
                // }
                else {
                    ...
                    match request {
                        ...
                        None => {
                            ...
                            let batch_size = match self.batch_size {
                                ...
                                None => {
                                    println!("当前的batch_size为none");
                                    40
                                },
                            };
                            println!("当前的batch_size为：{}",batch_size);
                            let mut requests = vec![];
                            // 此处改为共识参数
                            let start = self.round * batch_size;
                            let end = (self.round + 1) * batch_size;
                            for i in start..end {
...
pub enum MaliciousMode {
    ...
    LeaderDelaySendMessage(TriggerTimeInterval, Delay, ProtocolPhase),
    LeaderSendDuplicateMessages(TriggerTimeInterval, ProtocolPhase),
    ReplicaFeignDeath(TriggerTimeInterval, ProtocolPhase),
    ...
    ReplicaDelaySendMessage(TriggerTimeInterval, Delay, ProtocolPhase),
    ReplicaSendDuplicateMessage(TriggerTimeInterval, ProtocolPhase),
