use super::{
    log::{ConsensusLog, LogPhaseState},
    state::State,
};
...
use crate::pbft::message::{
    CheckPoint, Commit, MetaReply, NewView, PrePrepare, Prepare, ProofMessages, PublicKey, Reply,
    ViewChange,
};
...
impl PBFTProtocol {
    ...
    pub fn broadcast_viewchange(&mut self, current_peer_id: &[u8]) -> PhaseState {
        ...
        println!("viewchange:{:?}",viewchange.clone());
    ...
    pub fn handle_viewchange(&mut self, current_peer_id: &[u8], msg: &ViewChange) -> PhaseState {
        ...
        println!("record!");
        let msg_count = self.log.get_viewchange_messages_count_by_view(new_view);
        println!("count is :{}", msg_count);
        ...
        if (msg_count as u64 > self.state.fault_tolerance_count && msg_count < 3)
        {
        ...
        // if self.is_leader(current_peer_id.to_vec())
           if true && msg_count as u64 >= 2 * self.state.fault_tolerance_count
        {
            self.state.view += 1;
            println!("收到足够多的view_change消息，切换视图");
            // return self.broadcast_newview(new_view)
            // modify

            if self.state.timeout_flag {
                self.state.timeout_flag = false;
            return PhaseState::Over(Some(self.current_request()));
            }
            else {
                return PhaseState::ContinueExecute(send_type)
            }
    ...
    fn handle_request(&mut self, current_peer_id: &[u8], request: &Request) -> PhaseState {
        self.view_timeout_start(self.state.view);
        ...
        println!("StartView is :{:?}", start_msg);
        let start_serialized_msg = coder::serialize_into_json_bytes(&start_msg);
        send_query.push_back(SendType::Broadcast(start_serialized_msg));
        println!("发送StartView");
    ...
    fn handle_preprepare(&mut self, current_peer_id: &[u8], msg: &PrePrepare) -> PhaseState {
        ...
        self.state.current_request = m.clone();
    ...
    fn handle_commit(&mut self, current_peer_id: &[u8], msg: &Commit) -> PhaseState {
        ...
        if current_count as u64 == threshold {
            ...
            send_query.push_back(SendType::Unicast(
                PeerId::from_bytes(&self.state.primary).expect("msg"),
                serialized_msg,
            ));
    ...
    fn handle_reply(&mut self, _current_peer_id: &[u8], msg: &Reply) -> PhaseState {
        ...
        let reply_threshold =
            (self.state.node_count as u64 - self.state.fault_tolerance_count - 1) as u64;
        println!("cerrent key is : {:?}", key_str.clone());
        println!(
            "current_count is : {:?},threshold is : {:?}",
            current_count, reply_threshold
        );
    ...
    pub fn storage_public_key_by_peer_id(
        ...
        PhaseState::ContinueExecute(VecDeque::new())
        // let data = self.broadcast_viewchange(current_peer_id);
        // data
    }
}
...
impl ProtocolBehaviour for PBFTProtocol {
    ...
    fn consensus_protocol_message_handler(
        ...
        match message.msg_type {
            MessageType::StartView(view, request) => {
                println!("收到StartView");
                return self.handle_startview(&current_peer_id, view,request);
            }
            MessageType::Request(msg) => return self.handle_request(&current_peer_id, &msg),
            MessageType::PrePrepare(msg) => return self.handle_preprepare(&current_peer_id, &msg),
    ...
    fn protocol_phases(&mut self) -> HashMap<u8, Vec<u8>> {
        ...
        let preprepare_json = coder::serialize_into_json_str(&preprepare)
            .as_bytes()
            .to_vec();
        ...
        hash_map.insert(1, preprepare_json);
        hash_map.insert(2, prepare_json);
        hash_map.insert(3, commit_json);

        self.phase_map.insert(1, String::from("PrePrepare"));
        self.phase_map.insert(2, String::from("Prepare"));
        self.phase_map.insert(3, String::from("Commit"));
    ...
    fn view_timeout_handler(&mut self, current_peer_id: PeerId) -> PhaseState {
        ...
        let data = self.broadcast_viewchange(&current_peer_id.to_bytes());
        data
    }
    ...
    fn protocol_reset(&mut self) {
        ...
        self.state.view = 0;
        self.state.timeout_flag = false;
        self.log.messages = HashMap::new();
        self.log.viewchanges = HashMap::new();
        self.log.reply_messages = HashMap::new();
        self.log.requests = HashMap::new();
        self.log.request_map = HashMap::new();
        self.log.request_phase_state = HashMap::new();
    ...
    fn get_current_phase(&mut self, _msg: &[u8]) -> u8 {
        if _msg.len() == 0 {
            ...
            let i = match data.msg_type {
                ...
                MessageType::PrePrepare(_) => 1,
                MessageType::Prepare(_) => 2,
