        ...
        loop {
            tokio::select! {
                ...
                _ = self.executor.view_timeout_notify.notified() => {
                    ...
                    // send newview
                    self.executor.newview(&self.id.to_bytes()).await;
                ...
                Some(msg) = self.executor.msg_rx.recv() => {
                    ...
                    match message.msg_type {
                        MessageType::PrepareVote(_) | MessageType::PreCommitVote(_) | MessageType::CommitVote(_) | MessageType::NewView(_) => {
                            let leader_id = PeerId::from_bytes(&self.executor.state.primary).expect("Leader peer id error.");
                            swarm.behaviour_mut().unicast.send_message(&leader_id, msg);
                        }
                        // MessageType::PreCommitVote(_) => {
                        //     let leader_id = PeerId::from_bytes(&self.executor.state.primary).expect("Leader peer id error.");
                        //     swarm.behaviour_mut().unicast.send_message(&leader_id, msg);
                        // }
                        // MessageType::CommitVote(_) => {}
                ...
                event = swarm.select_next_some() => match event {
                    SwarmEvent::Behaviour(OutEvent::Unicast(UnicastEvent::Message(message))) => {
                        ...
                        // let _peer = PeerId::from_bytes(&message.source).expect("Source peer error.");
                    ...
                    SwarmEvent::Behaviour(OutEvent::Gossipsub(GossipsubEvent::Message {
                        ...
                        match msg.msg_type {
                            MessageType::Request(request) => {
                                ...
                                // *self.executor.state.hava_request.lock().await = true;
                            }
                            _ => {
                                self.executor.message_handler(&self.id.to_bytes(), &message.data).await;
                            }
