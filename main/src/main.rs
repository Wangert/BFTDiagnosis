...
use cli::args::Args;
use consensus::{hotstuff, pbft::{controller_node::ControllerNode, node::ConsensusNode}};
...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if is_consensus_node.eq("true") {
        let mut node = hotstuff::consensus_node::node::Node::new(Box::new(local_peer), &args.swarm_port.to_string());
        node.network_peer_start().await?;
    } else if is_consensus_node.eq("false") {
        let mut node = hotstuff::controller_node::node::Node::new(Box::new(local_peer), &args.swarm_port.to_string());
        node.network_peer_start().await?;
    } else {
